...
namespace Galaga.MovementStrategy {
       ...
       void IMovementStrategy.MoveEnemy(Enemy enemy)
        ...
        void IMovementStrategy.MoveEnemies(EntityContainer<Enemy> enemies)
        {
            foreach (Enemy e in enemies) {
                //e.Shape.AsDynamicShape().Direction = new Vec2F(0.0f, e.speed);
                e.Shape.MoveY(e.speed);
            }
        } 
    }
    ...
    public class ZigZagMove : IMovementStrategy {
        void IMovementStrategy.MoveEnemy(Enemy enemy)
        {
            ...
            enemy.Shape.Move(new Vec2F(x, enemy.speed));
        }

        void IMovementStrategy.MoveEnemies(EntityContainer<Enemy> enemies)
        {
            enemies.Iterate(enemy => {
                float y = enemy.Shape.Position.Y - enemy.speed;
